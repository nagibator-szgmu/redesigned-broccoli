      ...
      <div style={{ background: C.panelBg, backdropFilter: "blur(16px)", border: `1px solid ${C.border}`, borderRadius: 18, padding: "18px 14px", boxShadow: "0 4px 24px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          {[
            ...
            { value: avgScore, max: 100, label: t("progress.avgScore"), color: C.accent },
          ...
          sessionHistory.slice(0, 5).map((s) => {
            ...
            return (
              <div key={s.id} className="session-row" style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 10px", borderRadius: 12, marginBottom: 4, transition: "background 0.15s", cursor: "pointer" }}>
                ...
                <button onClick={() => startGame(s.caseId)} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, padding: "4px 12px", fontSize: 12, color: C.accent, cursor: "pointer", fontFamily: FONT, flexShrink: 0, fontWeight: 600 }}>{t("actions.retry")}</button>
              </div>
            );
          })
        )}
