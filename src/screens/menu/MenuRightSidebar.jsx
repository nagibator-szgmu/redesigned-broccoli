...
export default function MenuRightSidebar({
  ...
  return (
    <div style={{ width: 280, flexShrink: 0, overflowY: "auto", padding: "26px 20px 40px 4px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: C.panelBg, backdropFilter: "blur(16px)", border: `1px solid ${C.border}`, borderRadius: 18, padding: "18px 14px", boxShadow: "0 4px 24px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 11, color: C.accent, fontFamily: FONT, background: C.accentDim, borderRadius: 5, padding: "2px 8px" }}>{t("progress.streak")}</span>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          {[
            { value: casesPlayed, max: CASES.length, label: t("progress.cases"), color: C.accent },
            { value: avgScore, max: 100, label: t("progress.avgScore"), color: C.yellow },
      ...
      <button className="start-btn" onClick={startGame} style={{ background: `linear-gradient(135deg, ${C.accent}, ${C.yellow})`, border: "none", borderRadius: 14, padding: "16px", fontSize: 15, fontWeight: 800, color: theme === "dark" ? "#121316" : "#FFFFFF", cursor: "pointer", fontFamily: FONT, letterSpacing: 0.5, width: "100%", boxShadow: `0 6px 24px ${C.accent}35`, transition: "all 0.2s" }}>
        {t("cta.newPatient")}
      </button>

      {/* Recent Sessions List */}
      <div style={{ background: C.panelBg, backdropFilter: "blur(16px)", border: `1px solid ${C.border}`, borderRadius: 18, padding: "18px 16px", boxShadow: "0 4px 24px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: C.white, fontFamily: FONT }}>{t("sessions.recent")}</span>
          <span onClick={() => setShowAllCases(true)} style={{ fontSize: 11, color: C.accent, fontFamily: FONT, background: C.accentDim, borderRadius: 5, padding: "2px 8px", cursor: "pointer" }}>{t("sessions.allCases")}</span>
        </div>
          ...
          sessionHistory.slice(0, 5).map((s) => {
            ...
            return (
              <div key={s.id} className="session-row" style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 10px", borderRadius: 12, marginBottom: 4, transition: "background 0.15s", cursor: "pointer" }}>
                ...
                <button onClick={() => startGame(s.caseId)} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, padding: "4px 12px", fontSize: 12, color: C.accent, cursor: "pointer", fontFamily: FONT, flexShrink: 0 }}>{t("actions.retry")}</button>
              </div>
            );
          })
        )}
